...
import { deleteDivision, getAllEmployee, updateDivision } from '../store'


const Action = row => {  
  const dispatch = useDispatch()

  const {permission} = useSelector(state => state.division)
  ...
  const onSubmitDelete = (e) => {
    e.preventDefault()
    dispatch(deleteDivision(deleteItem.id))
    setDeleteShow(!deleteShow)
  }

  const onSubmitEdit = (e) => {
    e.preventDefault()
    dispatch(updateDivision(editData))
  ...
  return (
    <div className='column-action px-1'>
      ...
      >
        ...
        <div className='mb-1'>
          <Label className='form-label' for='select-plan'>
            Select Division Head <span className='text-danger'>*</span>
          </Label>
          <Select
                isClearable={false}
      ...
      >
        ...
        <ModalBody className='px-2 pb-2'>
          <div className='text-center mb-4'>
            <h1>Are you sure to delete '{deleteItem?.name}' Division?</h1>
          </div>
...
export const columns = [
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex justify-content-center'>
        <span>{row.departments.length}</span>
      </div>
      )
  },
  {
