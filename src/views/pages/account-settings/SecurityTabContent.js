...
const SecurityTabContent = () => {
  ...
  const [notMatch, setNotMartch] = useState(null)
  ...
  const onSubmit = async(data) => {
    ...
    const response = await axios.post(`/password-update`, data)
    if (response?.response?.data?.message) setNotMartch(response?.response?.data?.message)
    else navigate('/pages/profile')
  }
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Row>
              <Col sm='6' className='mb-1'>
                ...
                {notMatch && (
                  <FormFeedback className='d-block'>Current Password Does not Match!</FormFeedback>
                )}
              </Col>
            </Row>
            <Row>
